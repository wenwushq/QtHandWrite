...
public:
	...
	static long		m_inputWinId;
	static BstrResults m_pBstrResults;

    // Static method that creates an object of the class
	static bool Init(long inputWinId);
	static void SetBstrResultsCB(BstrResults pBstrResults);
	static void Clear();

    // Constructor
    CAdvRecoApp() :
		m_hwndSSGestLV(NULL), m_hwndMSGestLV(NULL),
